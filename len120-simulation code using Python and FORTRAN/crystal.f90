...
!
   integer,parameter::SLAB=120,W=150,Mtime=7!!W is the step interval for the impose,Mtime is the step interval for measuring
...
read(1,'(240E23.15)')slab_temperature_sum,slab_temperature_SQsum
if ( N <= 0 ) then
...
e=0.0877!the amplitude of the flux
...
!   call Grouping
   ...
   vel = vel + 0.5d0*deltat*acc                        ! v(t+dt)
!  call Grouping
! write(*,*) slab_label(1)!,'pos',pos(3,1)*BoxSize(3)
 !  write(*,*)'vel',vel(3,1)*Boxsize(3),'acc',acc(3,1)*BoxSize(3)
...
write(1,'(240E23.15)')slab_temperature_sum,slab_temperature_SQsum
