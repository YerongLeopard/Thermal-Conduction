...
#calculation
plt.plot(x,y,'.b')
plt.plot( x,k*x+b,'-r',label=('$\kappa=$'+str(round(kappa,2))+'('+str(round(kappa_devi,2))+')'))
...
x.append(20)
x=np.array(x)
plt.plot(x,k*x+b,'-r')#,label=('Original 2nd Fit:$\kappa=$'+str(round(-kappa,2))+'('+str(round(kappa_devi,2))+')'))
...
x1=np.arange(20000)
x1=x1/1000.0
plt.plot(x1,func(x1,T,T0),'-b',label='$\kappa=$'+'7.04(0.14)')
plt.errorbar(x,y,yerr=(err),linestyle="None")
x=np.arange(20)
#plt.xticks([i in range(20)],[i for i in range(20)])
plt.xticks([i for i in range(20)],[np.mod(i-10,20) for i in range(20)])
#plt.xticks([i in range(4)],['%i'%(10**i) for i in range(4)])
pl.legend(loc='best')
pl.xlabel('Slabs',fontsize=20)
pl.ylabel('Temperature',fontsize=20)
