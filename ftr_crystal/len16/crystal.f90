...
!
   integer,parameter::SLAB=16,W=15,Mtime=7!!W is the step interval for the impose,Mtime is the step interval for measuring
...
read(1,'(32E23.15)')slab_temperature_sum,slab_temperature_SQsum
if ( N <= 0 ) then
...
e=12391.80!the amplitude of the flux
...
number_of_atoms=0
do i=1,N
 slab_label(i)=floor((pos(DIM,i)+0.50000d0)*SLAB)
...
!!!comparision
  ...
  do i=0,SLAB/4-2!label of cold slabs:0,1,2,3
    ...
    real_vel=Boxsize*vel(:,max_indexA(i))
  enddo
  do i=SLAB*3/4,SLAB-1!label of cold slabs:15,16,17,18,19
    ...
    real_vel2=vel(:,min_indexB(SLAB*3/2-2-i))*BoxSize
  enddo
